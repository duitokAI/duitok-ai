...
function hydrateIconsSoon() {
  ...
  if (document.documentElement.classList.contains("is-route-changing")) {
    run();
    return;
  }
...
function imageGenerateConsole(p, selectedModel) {
  ...
  return `<section class="image-generate-console ${longPromptClass} ${generatingClass}" data-image-generate-console>
...
function resultPreview(item, options = {}) {
  ...
  const videoTriggerMedia = `<div class="result-video-shell ${options.wall ? "result-video-wall-preview" : ""}"><video class="result-video" src="${esc(videoSrc)}" preload="${videoPreload}" playsinline muted onloadedmetadata="${esc(ratioSync)}" onerror="${esc(videoPosterFallback)}"></video><span class="result-play-button">${icon("play", 26)}${options.wall ? "" : "<span>点击查看</span>"}</span><span class="result-video-poster-icon" aria-hidden="true">${icon("video", 30)}</span></div>`;
